...
-- 新增：记录每个群组每一周的冠军（周一到周日算一周），用来支持"永久成就勋章"——
-- 拿过第一名的人，这条记录会永远留着，累计次数就是这张表里按 user_id 数出来的行数。
...
create index if not exists weekly_champions_user_id_idx on weekly_champions(user_id);
